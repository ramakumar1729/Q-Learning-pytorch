    ...
    def fit(self, env, num_iterations, max_episode_length=None):
        ...
        while idx < num_iterations:
            ...
            if idx % 10000 == 0 and idx > self.num_burn_in:
                torch.save(Q.state_dict(), 'models/LQN-basic/'+self.model_name+'.Q.model.'+str(idx))
                torch.save(target_Q.state_dict(), 'models/LQN-basic/'+self.model_name+'.targetQ.model.'+str(idx))
                with open('models/LQN-basic/stats.pkl.'+str(idx),'wb') as fw:
...
if __name__ == "__main__":
    ...
    agent = LQNAgent(LQN,
                                ...
                                epsilon,
                                model_name)
    print('create LQN-basic agent')

    if mode == 'train':
        env = wrappers.Monitor(env, '/tmp/SpaceInvaders-LQN-basic-expt-train.'+model_name, force=True)
        agent.fit(env, num_training_samples)
    elif mode == 'eval':
        env = wrappers.Monitor(env, '/tmp/SpaceInvaders-LQN-basic-expt-eval.'+model_name, force=True)
